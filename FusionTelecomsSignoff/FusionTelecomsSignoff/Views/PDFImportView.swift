        ...
        VStack(spacing: 28) {
            ...
            VStack(spacing: 8) {
                Text("Import from PDF")
                    .font(.title2)
                    .fontWeight(.semibold)
                Text("Choose a customer order form PDF and the app will automatically read the job reference, customer name, address, and work description.")
        ...
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                if fullText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    readError = "No readable text found in this PDF. It may be a scanned image."
                } else {
    ...
    var body: some View {
        Form {
            ...
            Section("Job Reference") {
                TextField("Not detected", text: $fields.jobReference)
            ...
            Section("Site Address") {
                TextField("Not detected", text: $fields.address, axis: .vertical)
                    .lineLimit(2...4)
            }

            Section("Work Description") {
                TextField("Not detected", text: $fields.workDescription, axis: .vertical)
                    .lineLimit(3...8)
            }

            Section {
                Button("Apply to Job Form") {
                    onComplete(true, fields)
                }
                .fontWeight(.semibold)

                Button("Cancel", role: .cancel) {
                    onComplete(false, fields)
                }
                .foregroundStyle(.secondary)
            }
        }
